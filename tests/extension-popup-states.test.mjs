...
function runBrowserAssertions(baseUrl) {
  const script = path.join(projectRoot, 'tests', 'helpers', 'extension_popup_states.py')
  return new Promise((resolve, reject) => {
    ...
    child.once('exit', (code) => {
      if (code === 0) resolve()
      else reject(new Error(`연장 팝업 렌더링 검수가 실패했습니다.\n${stdout}${stderr}`))
    })
  })
}

test('실제 API 연장 팝업은 견적 로딩과 실패 상태를 노출하고 로딩 중 조작을 잠근다', { timeout: 90_000 }, async () => {
