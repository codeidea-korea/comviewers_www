...
test('U17 장바구니 확인 팝업은 저장 완료 후 PDF 구성으로 열린다', async () => {
  ...
  for (const source of [list, detail]) {
    ...
    assert.match(source, /장바구니 담기 완료/)
    assert.match(source, /상품을 장바구니에 담았습니다\.<br \/>장바구니로 이동하시겠습니까\?/)
    assert.match(source, /closeLabel="계속 쇼핑하기"/)
    assert.match(source, /confirmLabel="장바구니 이동"/)
  }
...
test('U22/U23 완료 화면은 PDF 밖 장식과 결제 결과 우회 링크를 표시하지 않는다', async () => {
  const [complete, payment, styles] = await Promise.all([
    ...
    readFile(new URL('../src/routes/commerce/-components/checkout/CheckoutPaymentWidget.tsx', import.meta.url), 'utf8'),
    readFile(new URL('../src/styles/commerce.css', import.meta.url), 'utf8'),
  ])

  assert.doesNotMatch(complete, /complete-check|>✓</)
  assert.doesNotMatch(payment, />결제 상태 확인</)
  assert.match(payment, /<CheckoutPaymentRefresh payment=\{payment\} \/>/)
  assert.match(styles, /\.complete-page--api \.commerce-page \{ height: auto; padding-bottom: 0; \}/)
