...
마이페이지와 커뮤니티의 토스트는 `src/components/ui/ToastControl.tsx`의 `Toast`와 `useToastMessage`를 사용한다. RCPC 접속정보 토스트 디자인을 공통 기준으로 하며 마크업·스타일·3초 노출 시간·등장/퇴장 애니메이션을 이 파일에서 함께 관리한다. 호출 시 `toastKey`도 전달해 같은 문구가 반복되어도 노출 시간을 다시 시작한다. 표시 중인 토스트는 전체 화면에서 하나로 제한하며 새 알림이 이전 알림을 대체한다. 이전 알림은 새 알림이 닫힌 뒤 다시 표시하지 않는다. 화면별 토스트 CSS나 타이머를 새로 만들지 않는다. 표시 조건·문구는 호출 기능에서 관리하고, 일반 인라인 상태 안내는 토스트와 구분한다.
