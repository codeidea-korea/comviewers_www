...
export function SessionControls({ loginLabel = '로그인' }: { loginLabel?: string } = {}) {
  ...
  if (session.status === 'anonymous') return <span className="session-controls"><Link to="/login">{loginLabel === '로그인' ? <TranslatedText id="nav.login" /> : loginLabel === '로그인하기' ? <TranslatedText id="auth.loginAction" /> : loginLabel}</Link>{auth.logoutNotice && <span role="status">{auth.logoutNotice}</span>}</span>
  if (session.status === 'password-change-required') return <span className="session-controls"><span>비밀번호 변경 필요</span><button type="button" onClick={() => { void auth.logout().catch(() => setError('로그아웃 상태를 다시 확인해 주세요.')) }}><TranslatedText id="nav.logout" /></button>{error && <span role="alert">{error}</span>}</span>
  ...
  return <span className="session-controls">
    <details className="session-user-menu" ref={menuRef}>
      ...
      <div>
        ...
        <button type="button" onClick={() => { void auth.logout().catch(() => setError('로그아웃 상태를 다시 확인해 주세요.')) }}><TranslatedText id="nav.logout" /></button>
      </div>
    </details>
    {error && <span role="alert">{error}</span>}
  </span>
...
function LoginRequiredModal() {
  ...
  return <Modal
    ...
    showClose={false}
    title="로그인이 필요합니다."
    titleTranslationKey="auth.loginRequired"
  >
...
export function SessionRouteGate({ children, roles, requireOrganization = false, recoveryPath }: {
  ...
  if (roles && !roles.includes(session.role)) return <main className="session-notice"><p><TranslatedText id="auth.accessDenied" /></p><SessionControls /></main>
...
export function CustomerCapabilityGate({ children }: { children: ReactNode }) {
  ...
  if (session.capabilityStatus !== 'ready' || !session.customerSession) return <main className="session-notice"><p role="alert">이용 권한을 확인하지 못했습니다.</p><button type="button" onClick={() => store.retryCustomerSession()}><TranslatedText id="common.retry" /></button><SessionControls /></main>
  ...
  if ((commerceRoute && !capability.commerceAvailable) || (managersRoute && !capability.cManagerManagementAvailable)) return <main className="session-notice"><p><TranslatedText id="auth.accessDenied" /></p><Link to="/mypage/rcpc">이용 RCPC로 이동</Link><SessionControls /></main>
  return children
}
