...
type CommentSectionProps = Pick<ReturnType<typeof usePostDetail>, 'commentQuery' | 'comments' | 'comment' | 'setComment' | 'commentMenuIndex' | 'setCommentMenuIndex' | 'setEditingCommentId' | 'addComment' | 'displayedCommentCount' | 'setCommentDeleteId' | 'commentAuthenticated'>

export function PostCommentSection({
  commentQuery, comments, comment, setComment, commentMenuIndex,
  setCommentMenuIndex, setEditingCommentId, addComment, setCommentDeleteId,
            ...
            {item.isMine ? <span className="comment-menu-anchor">
              ...
              {commentMenuIndex === index ? (
                <div className="comment-menu">
                  <button onClick={() => { setComment(item.content); setEditingCommentId(item.id); setCommentMenuIndex(null) }} type="button">수정</button>
