  ...
  return (
    <AppShell className="community-shell">
      {!plain ? <section className={`community-hero${support ? ' community-hero--support' : ''}`}>
        ...
        <div className="content-container"><h1>{title}</h1>{description ? <p>{description}</p> : null}</div>
      </section> : null}
      {children}
    </AppShell>
  )
...
export function CommunityTabs({ active }: { active: 'posts' | 'reviews' }) {
  return <nav aria-label="커뮤니티 구분" className="community-tabs">{communityTabs.map((tab) => <Link className={tab.id === active ? 'is-active' : ''} key={tab.id} to={tab.href}>{tab.label}</Link>)}</nav>
}
