...
export function ProfileEditor({ mutations, profile, withdrawal, currentPassword }: { mutations: ProfileMutations; profile: AccountProfileRead; withdrawal?: CustomerWithdrawalApi; currentPassword: string }) {
  ...
  return <><form className="profile-form profile-catalog" onSubmit={submit}>
    ...
    {validation ? <p className="mypage-notice mypage-notice--error" role="alert">{validation}</p> : null}
    {save.isError ? <p className="mypage-notice mypage-notice--error" role="alert">{save.error.message || '내 정보를 저장하지 못했습니다.'}</p> : null}
    {passwordChange.isError ? <p className="mypage-notice mypage-notice--error" role="alert">{passwordChange.error.message}</p> : null}
    {emailRequest.isError ? <p className="mypage-notice mypage-notice--error" role="alert">{emailRequest.error.message}</p> : null}
