...
export function ManagerEditorForm({ manager, pending, onSave, onClose, onDelete, checkLogin }: {
  ...
  return <form onSubmit={(event) => void submit(event)}><h2>{manager ? '담당자 정보' : '담당자 등록'}</h2>
    <fieldset disabled={pending} style={{ border: 0, padding: 0, minWidth: 0 }}>
      <ManagerField label="담당자명" help="1~10자의 한글, 영문, 숫자" value={draft.name} onChange={(event) => setDraft((current) => ({ ...current, name: event.target.value }))} />
      {manager ? <label className="manager-modal__field">아이디<input aria-label="아이디" readOnly value={draft.loginId} /></label> : <ManagerField label="아이디" help="5~16자의 영문, 숫자, 밑줄(_)" action={checking ? '확인 중' : checkedId && checkedId === draft.loginId.trim() ? '중복 없음' : '중복확인'} onAction={() => { if (!checking) void check() }} value={draft.loginId} onChange={(event) => { revision.current += 1; setCheckedId(''); setDraft((current) => ({ ...current, loginId: event.target.value })) }} />}
      <ManagerField label={manager ? '새 비밀번호' : '비밀번호'} showRequired={!manager} type="password" help={manager ? '변경할 때만 입력해 주세요.' : '8~16자의 영문, 숫자, 특수문자(!, @, #, $, %)'} value={draft.password} onChange={(event) => setDraft((current) => ({ ...current, password: event.target.value }))} />
      ...
      {manager && onDelete ? <button className="manager-modal__delete" type="button" onClick={onDelete}>담당자 삭제</button> : null}
      <DialogActions><button type="button" onClick={onClose}>{manager ? '닫기' : '취소'}</button><button disabled={pending} type="submit">{pending ? '저장 중' : manager ? '수정' : '담당자 등록'}</button></DialogActions>
    </fieldset>
  </form>
}
