...
export function AccountQueryState({ pending, error, retry }: { pending: boolean; error: Error | null; retry: () => unknown }) {
  ...
  if (error) return <div role="alert" className="mypage-notice mypage-notice--error"><p>{error.message || '정보를 불러오지 못했습니다.'}</p><button onClick={() => { void retry() }} type="button">다시 시도</button></div>
  return null
}
