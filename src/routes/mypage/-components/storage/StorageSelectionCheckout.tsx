...
export function StorageSelectionCheckout({ api, items }: { api: MyAccountReadServices; items: readonly AccountStorageItem[] }) {
  ...
  const [selected, setSelected] = useState<readonly string[]>(() => items.filter(item => item.status === 'stored' && (!item.paymentDueAt || new Date(`${item.paymentDueAt}+09:00`).getTime() > Date.now())).map(item => item.id))
  ...
  const move = useMutation({ mutationFn: () => {
    ...
    await Promise.all([client.invalidateQueries({ queryKey: ['my-account', 'read', 'storage'] }), client.invalidateQueries({ queryKey: ['cart'] })])
