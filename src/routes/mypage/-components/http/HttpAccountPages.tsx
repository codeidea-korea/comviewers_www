...
function HttpProfileEditor({ mutations, profile, withdrawal, currentPassword }: { mutations: ProfileMutations; profile: AccountProfileRead; withdrawal?: CustomerWithdrawalApi; currentPassword: string }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    if (Boolean(phoneMiddle) !== Boolean(phoneLast)) { setValidation('휴대폰 번호를 모두 입력해 주세요.'); return }
    if (Boolean(messengerType.trim()) !== Boolean(messengerId.trim())) { setValidation('메신저 종류와 아이디를 함께 입력해 주세요.'); return }
    if (wantsPasswordChange && !/^(?=.*[A-Za-z])(?=.*\d)(?=.*[!@#$%])[A-Za-z\d!@#$%]{8,16}$/.test(newPassword)) { setValidation('비밀번호는 8~16자의 영문, 숫자, 특수문자(!, @, #, $, %)를 사용해 주세요.'); return }
    if (wantsPasswordChange && newPassword !== newPasswordConfirm) { setValidation('비밀번호가 일치하지 않습니다.'); return }
    if (wantsPasswordChange && wantsEmailChange) { setValidation('비밀번호와 이메일 변경은 한 번씩 진행해 주세요.'); return }
    if (wantsEmailChange && (!emailLocal || !emailDomain)) { setValidation('변경할 이메일을 확인해 주세요.'); return }
  ...
  return <><form className="profile-form profile-catalog" onSubmit={submit}>
    ...
    <label className="profile-catalog__field"><span><b>*</b> 비밀번호</span><div className="profile-catalog__password"><input autoComplete="new-password" maxLength={16} onChange={event => setNewPassword(event.target.value)} type={showPassword ? 'text' : 'password'} value={newPassword}/><button aria-label={showPassword ? '비밀번호 숨기기' : '비밀번호 보기'} aria-pressed={showPassword} onClick={() => setShowPassword(value => !value)} type="button"><img alt="" src={eyeIcon}/></button></div><small>* 8~16자의 영문, 숫자, 특수문자(!, @, #, $, %)만 사용할 수 있습니다.</small></label>
