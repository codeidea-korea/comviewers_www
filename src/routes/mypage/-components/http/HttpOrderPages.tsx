...
import type { AccountOrderItem, AccountOrderQuery } from '@/api/myAccountOrders'
...
import { AccountInfo, ReadPages, accountDate, accountMoney, accountStatus, useAccountRead } from './AccountReadCommon'
import { HttpOrderItemActions, HttpOrderItemSupportActions } from './HttpOrderItemActions'
import { HttpOrderPaymentContinuation } from './HttpOrderPaymentContinuation'
import { HttpOrderProductDescription } from './HttpOrderProductDescription'
import { HttpOrderRefundSummary } from './HttpOrderRefundSummary'
...
const rowStatus = (item: AccountOrderItem, orderStatus: string, paymentStatus: string) => {
  if (item.refundPending) return '환불 접수'
  if (item.itemStatus === 'refunded' || orderStatus === 'refunded' || paymentStatus === 'refunded') return '환불완료'
  if (item.itemStatus === 'cancelled' || orderStatus === 'cancelled' || paymentStatus === 'cancelled') return '취소완료'
  if (paymentStatus === 'approved' && ['paid', 'completed'].includes(orderStatus)) return '결제완료'
  if (paymentStatus === 'pending' && orderStatus === 'payment_pending') return '입금대기'
...
export function OrdersPageContent({ api }: { api: MyAccountReadServices }) {
  ...
  const visibleGroups = Array.from(visibleItems.reduce((groups, row) => {
    const date = accountDate(row.order.orderedAt).slice(0, 10)
                  ...
                  <article className={`order-catalog-item order-item--pdf${terminal ? ' order-catalog-item--compact' : ''}`} key={item.orderItemId}>
                    ...
                    <div className="order-catalog-item__product">
                      {item.imageUrl && /^(https?:\/\/|\/[^/])/.test(item.imageUrl)
                        ? <div className="order-catalog-item__image"><img src={item.imageUrl} alt={item.title} /></div>
                        : <div className="order-catalog-item__image order-item__image-placeholder" aria-hidden="true">img</div>}
                      <div>
                        <div className="order-catalog-item__heading"><strong><span>품번</span> {item.productNo}</strong><small>{item.serverRoomName ?? '-'}</small>{item.rentalId ? <em className="order-catalog-item__state">{rentalStatusLabels[item.customerRentalStatus] ?? accountStatus(item.customerRentalStatus)}</em> : null}</div>
                        <HttpOrderProductDescription item={item} />
                        {item.billingUnit !== 'unit' ? (
                          <span className="order-catalog-item__period">{accountDate(item.serviceStartedAt).slice(0, 10)}~{accountDate(item.serviceEndsAt).slice(0, 10)}</span>
                        ) : <span className="order-catalog-item__period">수량 {item.quantity}개</span>}
                        {!terminal ? <p className="order-catalog-item__totals">
                          <span>주문금액 <b>{accountMoney(item.amount)}</b></span>
                          ...
                          <span><i aria-hidden="true" />최종 결제금액 <b>{accountMoney(finalItemAmount)}</b></span>
                        </p> : null}
                      </div>
                    </div>
                    <HttpOrderItemActions
                      api={api}
                      item={item}
                      paymentStatus={order.paymentStatus}
                      orderStatus={order.orderStatus}
                    />
                    <HttpOrderItemSupportActions
                      item={item}
                      paymentStatus={order.paymentStatus}
                      orderStatus={order.orderStatus}
                    />
                    {(category === 'cancelled_or_refunded' || item.refundPending || item.itemStatus === 'refunded')
                      ? <HttpOrderRefundSummary api={api} itemId={item.orderItemId} orderNo={order.orderNo} originalAmount={finalItemAmount} terminal={terminal} />
                      : null}
                    <HttpOrderPaymentContinuation
