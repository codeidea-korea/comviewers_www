...
export function CheckoutCustomer({ form, pending = false }: { form: CheckoutFormModel; pending?: boolean }) {
  ...
  return <section aria-busy={pending} className="checkout-customer">
    ...
    <CheckoutField error={touched.name && !/^[가-힣A-Za-z'-]{1,18}$/.test(contact.name.trim()) ? '이름은 1~18자의 한글, 영문, 하이픈, 아포스트로피만 사용할 수 있습니다.' : undefined} label="이름" required><input aria-label="주문자 이름" maxLength={18} required onChange={(event) => { setTouched(current => ({ ...current, name: true })); changeContact('name', event.target.value) }} value={contact.name} /></CheckoutField>
    <CheckoutField error={touched.email && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(`${contact.emailId}@${contact.emailDomain}`) ? '이메일 주소를 확인해 주세요.' : undefined} label="E-mail" required><div className="email-fields">
    ...
    <CheckoutField error={touched.phone && (contact.phoneMiddle || contact.phoneLast) && (!/^\d{3,4}$/.test(contact.phoneMiddle) || !/^\d{4}$/.test(contact.phoneLast)) ? '핸드폰 번호의 가운데 3~4자리와 끝 4자리를 확인해 주세요.' : undefined} label="핸드폰"><div className="phone-fields">
    ...
    <CheckoutField error={touched.messenger && Boolean(contact.messenger.trim()) !== Boolean(contact.messengerId.trim()) ? '메신저 종류와 아이디를 함께 입력해 주세요.' : undefined} label="메신저 ID"><div className="messenger-fields">
