...
export function CheckoutCustomer({ form, pending = false }: { form: CheckoutFormModel; pending?: boolean }) {
  ...
  return <section aria-busy={pending} className="checkout-customer">
    ...
    <CheckoutField error={form.fieldError('contact.phone') ?? (touched.phone && (contact.phoneMiddle || contact.phoneLast) && (!/^\d{3,4}$/.test(contact.phoneMiddle) || !/^\d{4}$/.test(contact.phoneLast)) ? '핸드폰 번호의 가운데 3~4자리와 끝 4자리를 확인해 주세요.' : undefined)} label="핸드폰"><div className="phone-fields">
