...
export function CheckoutDiscounts({ quote, couponId, points, disabled, pending, error, onCoupon, onPoints, onRetry }: Props) {
  ...
  return <section className="checkout-discounts" aria-busy={pending}>
    ...
    <CheckoutField label="쿠폰 할인"><div className="coupon-select-field"><NativeSelect aria-label="쿠폰 할인" disabled={disabled || pending || !quote?.coupons.length} value={couponId ?? ''} onChange={event => onCoupon(event.target.value ? Number(event.target.value) : null)}>
      <option value="">쿠폰 사용 안 함</option>{quote?.coupons.map(coupon => <option key={coupon.userCouponId} value={coupon.userCouponId}>{coupon.name} ({coupon.discountAmount.toLocaleString('ko-KR')}원 할인)</option>)}
    </NativeSelect></div>{quote && quote.coupons.length === 0 && <p>사용 가능한 보유 쿠폰이 없습니다.</p>}</CheckoutField>
    <p className="commerce-help">*사용 조건에 맞는 쿠폰만 선택할 수 있습니다.</p>
    <CheckoutField label="포인트 사용"><div className="field-with-button"><input aria-label="사용할 포인트" inputMode="numeric" disabled={disabled || !quote || quote.availablePoints === 0} value={points} onChange={event => { if (/^\d{0,10}$/.test(event.target.value)) onPoints(event.target.value) }} placeholder="0" /><button disabled={disabled || pending || !quote || maxPoints === 0} type="button" onClick={() => onPoints(String(maxPoints))}>전액사용</button></div><small className="commerce-points-owned"><span>보유</span><b>{quote ? quote.availablePoints.toLocaleString('ko-KR') : '-'} P</b></small>{quote?.availablePoints === 0 && <p>보유 포인트가 없습니다.</p>}</CheckoutField>
