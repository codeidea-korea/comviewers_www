...
function ColocationDraftForm({ initialDraft }: { initialDraft: ColocationDraft | null }) {
  const { filled, fixture, selectedFiles, removeFile, notice, validationIssue, completeOpen, emailDomain, setEmailDomain, emailDomainInput, setEmailDomainInput, phonePrefix, setPhonePrefix, messenger, setMessenger, pending, submitLabel, pendingLabel, completeTitle, completeMessage, updateFiles, submit, live, terms, termsPending, termsError, retryTerms, closeComplete } = useColocationForm(initialDraft)
  ...
  return (
    <AppShell className="colocation-shell">
      <section className={`colocation-page${filled ? ' colocation-page--compact' : ''}`}>
        <form className="colocation-card" onSubmit={submit}>
          ...
          <fieldset className={`colocation-consent${hasIssue('accepted') ? ' has-error' : ''}`}><legend className="sr-only">약관 동의</legend><label className={filled || hasIssue('accepted') ? 'has-error' : ''}><Checkbox key={terms?.id ?? 'preview'} name="accepted" disabled={live && !terms} required /> [필수] 입점 약관 동의</label>{filled ? <small>* 필수 약관에 동의해 주세요.</small> : issueMessage('accepted')}
            ...
            <div aria-label="입점 약관" className="colocation-terms-box" role="region" tabIndex={0} style={{ whiteSpace: 'pre-wrap' }}>{live ? terms ? terms.content : '현재 확인할 수 있는 입점 약관이 없습니다.' : '입점 약관 미리보기입니다. 실제 신청에는 게시된 입점 약관이 표시됩니다.'}</div>
          </fieldset>
          <button className="colocation-submit" disabled={pending || (live && !terms)} formNoValidate type="submit">{pending ? pendingLabel : submitLabel}</button>
